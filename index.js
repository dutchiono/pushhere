...
const contractAddress = process.env.MOONCATRESCUE_CONTRACT || '0x7A00B19eDc00fa5fB65F32B4D263CE753Df8f651';
//const providerURL = process.env.MOONCATRESCUE_RPC || https://bscrpc.com;
const NODE_URL = 'https://bscrpc.com'
const providerURL = new Web3.providers.HttpProvider(NODE_URL);
...
function loadMoonCatRescueContract(contractAddress){
...
function loadCatData (cb){
    ...
    async.parallel([
	function(cb){
	    mcrContract.methods.getCatIds({gasLimit:1000000}).call(tick(cb));
	},
	function(cb){
	    mcrContract.methods.getCatNames({gasLimit:1000000}).call(tick(cb));
	},
	function(cb){
	    mcrContract.methods.getCatOwners({gasLimit:1000000}).call(tick(cb));
	},
	function(cb){
	    mcrContract.methods.getCatOfferPrices({gasLimit:1000000}).call(tick(cb));
	},
	function(cb){
	    mcrContract.methods.getCatRequestPrice({gasLimit:1000000}).call(tick(cb));
